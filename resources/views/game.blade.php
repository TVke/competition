@extends('layout')
@section('content')
	@if($cookie && !session()->has('newChange'))
		<h1 class="heading-1 var-center">{{ __('app.no_double-title') }}</h1>
		<form action="{{ route('friend_invite',['player'=>$cookie->id]) }}" method="post" class="form">
			{{ csrf_field() }}
			{{ method_field('PATCH') }}
	...
	@elseif(session('ok'))
		...
		<form action="{{ route('friend_invite',['player'=>session('ok')]) }}" method="post" class="form">
			{{ csrf_field() }}
			{{ method_field('PATCH') }}
			<p class="body-text var-center">{{ __('app.friend-invite-info') }}</p>
			<label for="friend_email" class="label{{ $errors->has('friend_email') ? ' error' : '' }}">{{ __('app.friend-invite-label') }}</label>
	...
	@elseif(session('friend_added'))
		<h1 class="heading-1 var-center">{{ __('app.friend-added-title',['friend'=>session('friend_added')]) }}</h1>
		<p class="body-text var-center">{{ __('app.friend-added-info') }}</p>

	@elseif(session('retry'))
	...
	@else
		...
		<form action="{{ (!$cookie)?route('update_user'):route('second_play') }}" method="post" class="form">
			{{ csrf_field() }}
			{{ method_field('PUT') }}
			...
			@else
				<h3 class="body-text var-center">{{ __('app.second-play-info') }}</h3>
				<input type="submit" class="button var-submit var-center" value="{{ __('app.submit-button') }}">
